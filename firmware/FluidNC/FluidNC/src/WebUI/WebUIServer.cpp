...
namespace WebUI {
    ...
    namespace {
        struct LabelerConfig {
            ...
            float    glyphSpacing = 1.0f;
            uint16_t servoUp      = 90;
            uint16_t servoDown    = 35;
            uint16_t servoDelayMs = 180;
        };

        LabelerConfig readLabelerConfig() {
            Preferences prefs;
            LabelerConfig cfg;
            if (!prefs.begin("labeler", true)) {
                return cfg;
            }
            cfg.xSteps       = prefs.getFloat("x_steps", cfg.xSteps);
            cfg.ySteps       = prefs.getFloat("y_steps", cfg.ySteps);
            cfg.xMaxSpeed    = prefs.getFloat("x_speed", cfg.xMaxSpeed);
            ...
            cfg.glyphSpacing = prefs.getFloat("spacing", cfg.glyphSpacing);
            cfg.servoUp      = prefs.getUShort("servo_up", cfg.servoUp);
            cfg.servoDown    = prefs.getUShort("servo_down", cfg.servoDown);
            cfg.servoDelayMs = prefs.getUShort("servo_delay", cfg.servoDelayMs);
            prefs.end();
            return cfg;
        }

        void sendLabelerConfig(AsyncWebServerRequest* request, const LabelerConfig& cfg) {
            String json;
            json.reserve(420);
            json = "{\"version\":1,\"xStepsPerMm\":" + String(cfg.xSteps, 4);
            json += ",\"yStepsPerMm\":" + String(cfg.ySteps, 4);
            json += ",\"xMaxSpeedMmS\":" + String(cfg.xMaxSpeed, 3);
            ...
            json += ",\"glyphSpacingMm\":" + String(cfg.glyphSpacing, 3);
            json += ",\"servoUpAngle\":" + String(cfg.servoUp);
            json += ",\"servoDownAngle\":" + String(cfg.servoDown);
            json += ",\"servoDelayMs\":" + String(cfg.servoDelayMs) + "}";
            AsyncWebServerResponse* response = request->beginResponse(200, "application/json", json);
            response->addHeader("Cache-Control", "no-store");
            request->send(response);
        }

        bool labelerArg(AsyncWebServerRequest* request, const char* name, float& value) {
            if (!request->hasParam(name, true)) return false;
            value = request->getParam(name, true)->value().toFloat();
            return std::isfinite(value);
        }

        bool labelerArg(AsyncWebServerRequest* request, const char* name, uint16_t& value) {
            if (!request->hasParam(name, true)) return false;
            long parsed = request->getParam(name, true)->value().toInt();
    ...
    void WebUI_Server::handleLabelerConfigPost(AsyncWebServerRequest* request) {
        ...
        bool valid = labelerArg(request, "xStepsPerMm", cfg.xSteps) && labelerArg(request, "yStepsPerMm", cfg.ySteps) &&
                     ...
                     labelerArg(request, "servoDelayMs", cfg.servoDelayMs);
        ...
        if (!valid) {
            request->send(400, "application/json", "{\"error\":\"Parametros ausentes o fuera de rango\"}");
            return;
        }

        Preferences prefs;
        if (!prefs.begin("labeler", false)) {
            request->send(500, "application/json", "{\"error\":\"No se pudo abrir la memoria persistente\"}");
            return;
        }
        prefs.putFloat("x_steps", cfg.xSteps);
        prefs.putFloat("y_steps", cfg.ySteps);
        prefs.putFloat("x_speed", cfg.xMaxSpeed);
        ...
        prefs.putFloat("spacing", cfg.glyphSpacing);
        prefs.putUShort("servo_up", cfg.servoUp);
        prefs.putUShort("servo_down", cfg.servoDown);
        prefs.putUShort("servo_delay", cfg.servoDelayMs);
        prefs.end();
        sendLabelerConfig(request, cfg);
    }
